<script lang="ts">
	...
	{#if $queryMessage.isSuccess && $queryMessage.data}
							...
							{#if messages.length > 0}
								...
								<div class="flex flex-col gap-2">
									{#each messages as msg (msg.created_at)}
										...
										<div
											class="max-w-lg {isYourMessage ? 'self-end' : ''}"
										>
											<div
												class="rounded-lg px-4 py-2 {isYourMessage ? 'bg-app-blue-50' : 'bg-app-blue-100'}"
											>
												{msg.text}
											</div>
											<p
		...
		<div class="flex gap-2 px-16 py-6 font-title outline outline-2 outline-sky-900">
			...
			<button on:click={sendMessage} class="flex items-center justify-center rounded-3xl bg-app-blue-600 px-12 text-2xl text-white">
				SEND
			</button>
		</div>
	{:else}
