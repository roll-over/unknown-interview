	...
	{#if $queryChats.data && $queryChats.data.length > 0}
		{#each $queryChats.data as { chat_id, chat_name, last_message_text, own }}
			<li>
				...
				>
					<div class="rounded-lg border-2 border-app-blue-100 p-3 dark:border-app-dark-light">
						<span class="text-xl">{chat_name}</span>
						{#if own}
							<span
								class="block w-full overflow-x-hidden text-ellipsis whitespace-nowrap font-normal transition-[font-weight]"
							>
						...
						{:else}
							<span
								class="block w-full overflow-x-hidden text-ellipsis whitespace-nowrap font-bold transition-[font-weight]"
							>
								{last_message_text}
							</span>
						{/if}
