	...
	<div class="rounded-md bg-app-blue-50 px-2 py-1 dark:bg-app-dark-gray">
		<div class="flex flex-wrap gap-2">
			{#each selectedSkills as skill, i (skill)}
				<button
					class="chip bg-app-blue-100 px-4 py-0.5 md:text-xl text-base dark:bg-app-dark-light"
					type="button"
					on:click={() => {
			...
			{/each}
			<input
				class="w-28 rounded bg-transparent px-4 py-0.5 pl-3 md:text-xl text-base caret-app-blue-600 dark:outline-white"
				bind:value={newSkill}
				on:keydown={(e) => {
					if (e.key === 'Enter') {
						e.preventDefault();
