...
>
	...
	<svelte:fragment slot="footer">
		<button
			aria-label="dislike"
			class="aspect-square  h-20 md:h-32 -scale-100 rounded-full bg-app-blue-600 p-4 text-white transition-colors current:bg-red-400 dark:bg-app-dark-blue dark:current:bg-red-400"
		...
		<button
			aria-label="like"
			class="aspect-square h-20 md:h-32 rounded-full bg-app-blue-600 p-4 text-white transition-colors current:bg-green-400 dark:bg-app-dark-blue dark:current:bg-green-400"
			on:click={handleLike}
		>
			<ThumbsUpIcon class="h-full w-full" />
		</button>
