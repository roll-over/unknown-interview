...
from app.api.schemas.vacancy_note import VacancyNoteRequestSchema, VacancyNotesResponseSchema
from app.models.mixin import Note
from app.models.vacancy_note import VacancyNote

vacancy_note_router = APIRouter(prefix="/vacancy_note", tags=["Vacancy Notes"])


@vacancy_note_router.post("/", response_model=Note)
...
@vacancy_note_router.get("/{vacancy_id}/", response_model=VacancyNotesResponseSchema)
...
async def delete_vacancy_notes(vacancy_id: UUID):
    ...
    if not vacancy_note:
        return JSONResponse(content={"error": "There is no Notes with such ID"}, status_code=404)
